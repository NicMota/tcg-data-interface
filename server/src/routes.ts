...
import { load_cards_data, get_cards, update_prices, get_price, get_card, filter_cards } from './model.js';
...
{   
  ...
  const data = await get_cards(page,limit);
  res.send(data);
})
...
{ 
  ...
  const data = await filter_cards(filter);
  res.send(data);
